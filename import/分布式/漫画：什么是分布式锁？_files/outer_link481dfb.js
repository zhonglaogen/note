...
!!e.changeHref&&!/^https?:\/\/mp\.weixin\.qq\.com\/cgi-bin\//.test(g)&&(g=e.changeHref(g,f)),
r.on(d,"tap",function(e){
e.preventDefault();
var i="";
c[d.getAttribute("data-itemshowtype")]&&(i=c[d.getAttribute("data-itemshowtype")]);
var r=document.getElementById("js_link_dialog_name");
if(r.innerText="即将打开新的页面",u[j]&&u[j].subject_name&&"0"===u[j].item_show_type&&u[j].title?r.innerText='即将打开公众号 "'+u[j].subject_name+'" 的'+c[u[j].item_show_type]+"《"+u[j].title+"》":u[j]&&u[j].subject_name&&u[j].item_show_type>=0&&(r.innerText='即将打开公众号 "'+u[j].subject_name+'" 的'+c[u[j].item_show_type]),
...
_=function(){
return u[j]&&u[j].item_show_type>=0&&g.indexOf("mp.weixin.qq.com")>-1&&(m.isIOS||m.isAndroid)&&!m.isInMiniProgram?1==d.getAttribute("clicked")?!1:(a.invoke("openWebViewUseFastLoad",{
url:g,
...
}),!1):void(m.isAndroid||m.isIOS?p(g,{
sample:1,
reject:function(){
location.href=g;
}
}):location.href=g);
},"1"===w&&s.report([2,s.getRedirectType(j),"",img_popup?1:0,window.source,s.getUrlData(j)]),
("1"===w||o(d)||n(d))&&img_popup){
console.log("tap img link");
var l=document.getElementById("js_link_dialog");
l.style.display="block",l._url=j,(o(d)||n(d))&&((new Image).src=location.protocol+"//mp.weixin.qq.com/mp/jsmonitor?idkey=110809_8_1&r="+Math.random(),
l._type="OTHER"),document.querySelector("body").addEventListener("touchmove",t,{
passive:!1
});
}else _();
return!1;
